...
$GLOBALS['dlbr_eid_wc_vies_mock'] = array();
...
function wp_remote_post($url, $args) {
    $GLOBALS['dlbr_eid_wc_vies_mock']['url'] = $url;
    $GLOBALS['dlbr_eid_wc_vies_mock']['args'] = $args;
    return isset($GLOBALS['dlbr_eid_wc_vies_mock']['response'])
        ? $GLOBALS['dlbr_eid_wc_vies_mock']['response']
...
require_once dirname(__DIR__) . '/includes/class-dlbr-eid-wc-vies-client.php';
...
$de = DLBR_EID_WC_VIES_Client::parse_vat_number(' de 123 456 789 ');
vies_check(is_array($de) && 'DE' === $de['country_code'] && '123456789' === $de['vat_number'], 'normalizes a prefixed VAT number');

$el = DLBR_EID_WC_VIES_Client::parse_vat_number('GR123456789');
vies_check(is_array($el) && 'EL' === $el['country_code'], 'maps Greece to VIES country code EL');

$xi = DLBR_EID_WC_VIES_Client::parse_vat_number('XI123456789');
vies_check(is_array($xi) && 'XI' === $xi['country_code'], 'accepts Northern Ireland VAT prefix XI');

vies_check(null === DLBR_EID_WC_VIES_Client::parse_vat_number('GB123456789'), 'does not send GB numbers to VIES');
vies_check(null === DLBR_EID_WC_VIES_Client::parse_vat_number('DE-'), 'rejects an empty VAT number');

$valid = DLBR_EID_WC_VIES_Client::interpret_response(200, json_encode(array(
    'valid' => true,
...
$invalid = DLBR_EID_WC_VIES_Client::interpret_response(200, '{"isValid":false,"userError":"INVALID"}');
vies_check('invalid' === $invalid['status'], 'distinguishes a completed invalid result');

$unavailable = DLBR_EID_WC_VIES_Client::interpret_response(200, '{"isValid":false,"userError":"MS_UNAVAILABLE"}');
vies_check('unavailable' === $unavailable['status'], 'does not mistake a Member State outage for an invalid VAT ID');

$malformed = DLBR_EID_WC_VIES_Client::interpret_response(200, '{"valid":"true"}');
vies_check('unavailable' === $malformed['status'], 'rejects string booleans in API responses');

$bad_json = DLBR_EID_WC_VIES_Client::interpret_response(200, 'not JSON');
vies_check('unavailable' === $bad_json['status'], 'treats malformed API output as unavailable');

$http_error = DLBR_EID_WC_VIES_Client::interpret_response(503, '{"valid":false}');
vies_check('unavailable' === $http_error['status'], 'treats non-success HTTP status as unavailable');

$GLOBALS['dlbr_eid_wc_vies_mock']['response'] = array('response' => array('code' => 200), 'body' => '{"valid":true}');
$client = new DLBR_EID_WC_VIES_Client();
$checked = $client->check('GR123456789', 'DE111222333');
$payload = json_decode($GLOBALS['dlbr_eid_wc_vies_mock']['args']['body'], true);
...
vies_check('https://ec.europa.eu/taxation_customs/vies/rest-api/check-vat-number' === $GLOBALS['dlbr_eid_wc_vies_mock']['url'], 'calls the fixed Commission VIES endpoint');

$previous_url = $GLOBALS['dlbr_eid_wc_vies_mock']['url'];
$invalid_input = $client->check('GB123456789');
vies_check('invalid_input' === $invalid_input['status'] && $previous_url === $GLOBALS['dlbr_eid_wc_vies_mock']['url'], 'does not send unsupported input to VIES');

$GLOBALS['dlbr_eid_wc_vies_mock']['response'] = array('wp_error' => true);
